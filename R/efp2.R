efp2 <- function(formula, data = list(), spatial = FALSE, family = "SAR",listw, weights,
...
plot.efp2 <- function(x, alpha = 0.05, alt.boundary = FALSE, boundary = TRUE,
...
pvalue.efp2 <- function(x, lim.process, alt.boundary, functional = "max", h = NULL, k = NULL)
{
                                                                                                                             ...
                                                                                                                             "maxL2", "meanL2"), from = 0.15, to = NULL, point = 0.5, asymptotic = FALSE, data = list(), ...)
{
  ...
  {
    process <- efp2(formula, type = type, h = h, data = data, ...)
...
sctest.efp2 <- function(x, alt.boundary = FALSE, functional = c("max", "range", "maxL2", "meanL2"), ...)
{
  h <- x$par
  type <- x$type
  ...
  PVAL <- pvalue.efp2(STAT, lim.process, alt.boundary, functional = functional, h = h, k = k)
...
boundary.efp2 <- function(x, alpha = 0.05, alt.boundary = FALSE, functional = "max", ...)
{
  h <- x$par
  type <- x$type
  lim.process <- x$lim.process
  ...
  bound <- uniroot(function(y) {pvalue.efp2(y, lim.process, alt.boundary = alt.boundary,
...
lines.efp2 <- function(x, functional = "max", ...)
{
