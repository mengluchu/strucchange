           ...
           CritvalTable=NULL, rescale=NULL, border=NULL, fittednew=f1,...)
  {
    ...
    switch(obj$type,
           ...
           "OLS-CUSUM" = {
             ...
             computeEmpProc <- function(X, y,fittednew)
             {
               as.vector(cumsum((y - fittednew)/(sigmahat*sqrt(histsize))))
             }
             computeEstims <- NULL
             functional <- "max"
             if(is.null(border)){
           ...
           "OLS-MOSUM" = {
             ...
             computeEmpProc <- function(fittednew, X,y)
             {
               e <- as.vector(y - fittednew)
    ...
    obj <- list(breakpoint=NA, last=obj$nobs, process=NULL,
                ...
                type=obj$type, data=data, histtsp=obj$datatsp,fittednew=fittednew)
